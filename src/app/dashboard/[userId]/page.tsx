...
const MarkerRow = ({ label, data, userData }: { label: string, data: BloodMarker[], userData?: UserData | null }) => {
  ...
  const getStatusInfo = (value: number) => {
    ...
    // Special case for platelets with custom ranges (only optimal and abnormal)
    if (configKey === 'platelets') {
      if (value >= 140 && value <= 400) {
        return 'Optimal';
      } else {
        return 'Abnormal';
      }
    ...
    // Special case for fasting insulin with custom ranges
    if (configKey === 'fastinginsulin') {
      if (value > 18.4) {
        return 'Abnormal';
      } else if (value >= 3.57 && value <= 10.18) {
        return 'Optimal';
      } else if (value < 3.57 || (value > 10.18 && value <= 18.4)) {
        return 'Normal';
      } else {
        return 'Abnormal';
      }
    ...
    // Special case for AST with custom ranges
    if (configKey === 'ast') {
    ...
    // Special case for GGT with custom ranges
    if (configKey === 'ggt') {
      if (value < 3 || value > 70) {
        return 'Abnormal';
  ...
  if (configKey === 'totalcholesterol') {
    ...
    normalText = '5.1-5.7';
    optimalText = '3.0-5.1';
  } else if (configKey === 'fastinginsulin') {
    optimalMin = 3.57;
    optimalMax = 10.18;
    normalMin = null; // Complex range: <3.57 OR 10.18-18.4
    normalMax = null;
    abnormalText = '>18.4';
    ...
    normalText = '25-40';
    optimalText = '10.0-25.0';
  } else if (configKey === 'ggt') {
    optimalMin = 3;
    optimalMax = 21;
    normalMin = 21;
    normalMax = 70;
