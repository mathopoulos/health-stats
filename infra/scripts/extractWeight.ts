...
async function extractWeight() {
  ...
  const outputPath = path.join(dataDir, 'weight.json');
  ...
  const transform = new Transform({
    transform(chunk: Buffer, encoding: BufferEncoding, callback: (error?: Error | null, data?: any) => void) {
      try {
        ...
        const recordRegex = /<Record type="HKQuantityTypeIdentifierBodyMass"[^>]*sourceName="[^"]*"[^>]*unit="[^"]*"[^>]*startDate="[^"]*"[^>]*value="[^"]*"[^>]*>/g;
        const matches = currentChunk.match(recordRegex) || [];
        
        console.log(`Found ${matches.length} potential weight records in current chunk`);
          ...
          if (dateMatch && valueMatch && sourceNameMatch && unitMatch) {
            ...
            if (recordDate >= fourYearsAgo) {
              ...
              if (!isNaN(value)) {
                ...
                if (recordCount % 10 === 0) {
                  console.log(`Found ${recordCount} weight records from the last 4 years...`);
                }
              }
            }
          }
      ...
      readStream
        ...
        .on('finish', () => {
          if (records.length > 0) {
            ...
            console.log(`Successfully extracted ${records.length} weight records from the last 4 years to data/weight.json`);
